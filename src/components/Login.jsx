...
export default function SignInSide({setIsAlive}) {
  ...
  const handleSubmit = async(event) => {
    ...
    console.log(signIn);
    await Axios.post('http://localhost:5000/api/admin/login',signIn)
    .then(async(res)=>{
        ...
        console.log('-----------------------------------------');
        await localStorage.setItem('User',JSON.stringify(res.data.admin));
        await localStorage.setItem('Auth-token',JSON.stringify(res.data.authtoken));
        await localStorage.setItem('role',JSON.stringify(res.data.admin.role));
